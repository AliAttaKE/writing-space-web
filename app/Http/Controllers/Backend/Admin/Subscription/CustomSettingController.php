...
class CustomSettingController extends Controller
{
    ...
    public function custom_setting_package(Request $request)
    {
        // $pricing = Pricing::all();


        $pricing = PricingPakage::latest()
        ->when($request->text != null, function ($q) use ($request) {
            return $q->where('text', $request->text);
        })
        ->when($request->page_limit != null, function ($q) use ($request) {
        ...
        return view('backend.admin.subscription.custom_variation_pakage',compact('pricing'));
    }
    public function custom_setting_order(Request $request)
    {
        // $pricing = Pricing::all();


        $pricing = PricingOrder::latest()
        ->when($request->text != null, function ($q) use ($request) {
            return $q->where('text', $request->text);
        })
        ->when($request->page_limit != null, function ($q) use ($request) {
        ...
        return view('backend.admin.subscription.custom_variation_order',compact('pricing'));
    }
    ...
    public function custom_edit_order(Request $request, $id)
    {
    ...
    public function custom_edit_pkg(Request $request, $id)
    {
