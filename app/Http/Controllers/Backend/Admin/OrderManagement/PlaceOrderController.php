      ...
      public function updateStatus(Request $request)
{
    ...
    public function revision_order(Request $request)
    {
        ...
        $order = Orders::join('users', 'orders.user_id', '=', 'users.id') // Join with the users table
        ->select('orders.*', 'users.name as user_name','users.email as user_email')->where('order_status', 'Revision')
            ->when($request->order_id != null, function ($q) use ($request) {
                return $q->where('order_id', $request->order_id);
            })
            ->when($request->topic != null, function ($q) use ($request) {
    ...
    public function completed_order(Request $request)
    {
        ...
        $order = Orders::join('users', 'orders.user_id', '=', 'users.id') // Join with the users table
        ->select('orders.*', 'users.name as user_name','users.email as user_email')->where('order_status', 'Completed')
            ->when($request->order_id != null, function ($q) use ($request) {
                return $q->where('order_id', $request->order_id);
            })
            ->when($request->topic != null, function ($q) use ($request) {
    ...
    public function delivered_order(Request $request)
    {
        ...
        ->select('orders.*', 'users.name as user_name','users.email as user_email')->where('order_status', 'Delivered')
            ->when($request->order_id != null, function ($q) use ($request) {
                return $q->where('order_id', $request->order_id);
            })
            ->when($request->topic != null, function ($q) use ($request) {
...
public function order_files_for_writer_revision(Request $request)
{
    // 1) Validate
    $validator = Validator::make($request->all(), [
        'email' => 'required|email',
    ...
    // 4) Fetch orders
    $orders = Orders::where('order_status', 'Revision')
        ->select(
            'id', 'order_id', 'subject', 'description', 'academic_level',
            'type_of_paper', 'paper_format', 'order_status',
            'language_spelling', 'number_of_pages', 'powerpoint_slide',
