...
<script>
    ...
    var words = description.trim().split(/\s+/);
        ...
        Swal.fire('Error', 'Only 9000 words allowed in text area "Description & Detailed Specifications"', 'error');
        ...
        if (!isNull) {
            ...
            var url = '{{ route('customer.custom.subscription.store.free') }}';
        ...
        window.location.href = response.redirect_url; // 👈 backend se jo URL aaya hai usi pe bhej do
    } else {
        Swal.fire({
            icon: 'error',
            title: 'Error!',
