                        ...
                        <table cellspacing="0" cellpadding="0" width="100%" style="font-family:Arial,sans-serif;line-height:1.3em;margin:15px 0;overflow:hidden;width:100%;background:#131313;color:#777;font-style:italic;border-radius:4px;border-bottom:1px solid #c9c9c9;border-left:1px solid #c9c9c9;border-right:1px solid #c9c9c9">
                            ...
                            <tr style="font-family:Arial,sans-serif;line-height:1.3em">
                                <td style="vertical-align:middle;word-wrap:break-word;padding:15px 12px;font-family:Arial,sans-serif;text-align:left;text-transform:capitalize;font-size:14px;font-weight:normal;padding-top:7px;padding-bottom:7px;margin:0;line-height:1.4em;border-top:1px dotted #c9c9c9">
                                    ...
                                    <p> Order ID : {{($details['data']['order'][0]['id'])}} </p>
                                    <p> Title : {{($details['data']['order'][0]['erp_subject_name'])}} </p>
                                    <p> Description :{!!  ($details['data']['order'][0]['erp_order_message']) !!}</p>
                                    <p> Topic :{{($details['data']['order'][0]['erp_order_topic'])}}</p>
                                    <p> Paper Type: {{($details['data']['order'][0]['erp_paper_type'])}}</p>
                                    <p> Paper Format: {{($details['data']['order'][0]['erp_paper_format'])}}</p>
                                    <p> Language name : {{($details['data']['order'][0]['erp_language_name'])}}</p>
                                    <p> No of Pages : {{($details['data']['order'][0]['erp_number_Pages'])}}</p>
                                    <p> Spacing {{($details['data']['order'][0]['erp_spacing'] == '1' ? 'Double Spacing' : 'single_spacing')}}</p>
                                    <p> Sources: {{($details['data']['order'][0]['erp_copy_sources'])}}</p>
                                    <p>  Page Summary :{{($details['data']['order'][0]['erp_page_summary'])}}</p>
                                    <p> Statistical Analysis : {{($details['data']['order'][0]['erp_statistical_analysis'])}} </p>
